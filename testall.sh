...
# Regression testing script for LOL
...
Usage() {
    echo "Usage: testall.sh [options] [.lol files]"
...
CheckFail() {
    ...
    RunFail "$LOL" "<" $1 "2>" "${basename}.err" ">>" $globallog &&
...
then
    ...
    files="tests/test-*.lol tests/fail-*.lol"
fi
